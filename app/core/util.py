...
def application_location():
    """Get the path to a main script file no matter how it's run.

    Returns:
        str: path to a current python script
    """
...
def copy_file(source, destination):
    ...
    Args:
        source (str): path of a file to be copied
...
def default_key(obj, key, default=None):
    """Get value by attribute in the object or by key in dict
    if property exists returns value otherwise return `default` value.
    If the object does not exist or the object doesn't have property, returns `default`
...
def is_builtin(type_object):
    """Check if the object type is built-in
...
def int_or_default(value: Any, default: int = 0) -> int:
    """Convert value to int or return default value"""
...
def float_or_default(value: Any, default: float = 0) -> float:
    """Convert value to float or return default value"""
