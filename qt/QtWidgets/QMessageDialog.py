...
from app.core.util import OS_MAC
...
class QMessageDialog(QtWidgets.QDialog):
    ...
    def __ui__(self):
        ...
        self._ui_icon.setAlignment(QtCore.Qt.AlignmentFlag.AlignCenter)
        ...
        self._ui_layout.addWidget(self._ui_title, 0, QtCore.Qt.AlignmentFlag.AlignTop)
        self._ui_layout.addWidget(self._ui_text, 1, QtCore.Qt.AlignmentFlag.AlignTop)
        self._ui_layout.addWidget(self._ui_buttons, 0, QtCore.Qt.AlignmentFlag.AlignBottom)

        self.setLayout(self._ui_layout)
        self._ui_icon.raise_()
        self.setWindowTitle(self._title if not OS_MAC else "")
        self.setMinimumSize(360, 60)
        self.setWindowFlags(QtCore.Qt.WindowType.WindowCloseButtonHint | QtCore.Qt.WindowType.MSWindowsFixedSizeDialogHint)
        self._update_size()
    ...
    def setIcon(self, icon):
        ...
        if icon in QMessageDialog.Icon or icon is None:
            standard_icon = QtWidgets.QStyle.StandardPixmap.SP_MessageBoxInformation

            if icon == QMessageDialog.Information:
                standard_icon = QtWidgets.QStyle.StandardPixmap.SP_MessageBoxInformation
            elif icon == QMessageDialog.Question:
                standard_icon = QtWidgets.QStyle.StandardPixmap.SP_MessageBoxQuestion
            elif icon == QMessageDialog.Warning:
                standard_icon = QtWidgets.QStyle.StandardPixmap.SP_MessageBoxWarning
            elif icon == QMessageDialog.Critical:
    ...
    def buttonRole(self, button):
        ...
        return QMessageDialog.InvalidRole
    ...
    def addButton(self, button, role=None):
        ...
        if isinstance(button, str):
            name = button
            value = -1
        elif button in QMessageDialog.StandardButton:
            name = QMessageDialog.StandardButtonRoleName[button][1]
            role = button
            value = button
    ...
    def setStandardButtons(self, buttons):
        ...
        for button in buttons:
            if button not in QMessageDialog.StandardButton:
    ...
    def warning(parent=None, title="Warning", text=""):
        ...
        buttons = [QMessageDialog.Ok]

        return QMessageDialog(parent, title, text, QMessageDialog.Warning, buttons)
    ...
    def critical(parent=None, title="Critical Problem", text=""):
        ...
        return QMessageDialog(parent, title, text, QMessageDialog.Critical, buttons)
    ...
    def question(parent=None, title="Are you sure?", text=""):
        ...
        return QMessageDialog(parent, title, text, QMessageDialog.Question, buttons)
    ...
    def information(parent=None, title="Information", text=""):
        ...
        return QMessageDialog(parent, title, text, QMessageDialog.Information, buttons)
